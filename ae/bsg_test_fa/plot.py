...
std_attn_df = pd.read_csv(
    f"A100_prefill_stdattn_roofline.csv",
    header=None,
    names=categories,
)

fa_attn_df = pd.read_csv(
    f"A100_prefill_flashattn_roofline.csv",
    header=None,
    names=categories,
)

dfs = [std_attn_df, fa_attn_df]
print(dfs[0])
print(dfs[1])
...
for x, row_index in enumerate(["STD", "FA"]):
